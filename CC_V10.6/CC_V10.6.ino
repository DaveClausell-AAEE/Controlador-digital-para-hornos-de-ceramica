...
const char* menuAjustesItems[] = {"Info WiFi", "Calibracion"};
...
void procesarEntrada(bool subiendo, bool bajando, bool ok, bool exit) {
    ...
    switch (estadoActual) {
      ...
      case CONFIRMACION_INICIO:
        ...
        break;
      case MENU_AJUSTES:
        if (subiendo) menuAjustesCursor = (menuAjustesCursor - 1 + 2) % 2;
        if (bajando) menuAjustesCursor = (menuAjustesCursor + 1) % 2;
        if (exit) estadoActual = MENU_PRINCIPAL;
        if (ok) {
...
void dibujarPantallaInfoWiFi() {
  tft.setTextColor(TFT_CYAN); tft.setTextSize(2); tft.setCursor(20, 60); tft.print("INFO DISPOSITIVO");
  tft.setTextColor(TFT_WHITE); tft.setCursor(20, 100); tft.print("MAC: "); tft.print(WiFi.macAddress());
  tft.setCursor(20, 130); tft.print("WiFi: No Conectado");
