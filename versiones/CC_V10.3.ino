// =================================================================
// ==      CONTROLADOR DIGITAL PARA HORNO DE CERÁMICA V10.3       ==
// =================================================================
//      (Integración de Sensor MAX31855 y Mejora de Interfaz UI)
//
...
// Pinout MAX31855 (Hardware SPI VSPI compartido con TFT)
#define MAXDO   19
#define MAXCS   5
#define MAXCLK  18

// Inicializar MAX31855 usando Hardware SPI (solo definimos CS, usa SCK/MISO default)
...
void setup() {
  ...
  if (!thermocouple.begin()) {
    Serial.println("ERROR: MAX31855 no encontrado.");
  }
...
void leerSensores() {
  ...
  if (isnan(c)) {
    // Si falla el sensor, usamos el modo SIMULACIÓN
    if (millis() - ultimoUpdateTempSimulada > 500) {
      ultimoUpdateTempSimulada = millis();
      // Física simplificada de calentamiento/enfriamiento
      if (digitalRead(RELAY_PIN) == LOW) { // Relé activo (LOW)
        temperaturaCrudaSimulada += 1.0; 
      } else { 
        if (temperaturaCrudaSimulada > 27.2) { temperaturaCrudaSimulada -= 0.2; } 
      }
      necesitaRefresco = true;
    }
    ...
    temperaturaCrudaSimulada = c; // Mantenemos la variable sincronizada
    
    // Refrescar pantalla si la temperatura cambia en 0.5 grados
...
void dibujarLineaDeEtapa(int idx) {
  ...
  if (idx == -1) {
    ...
    uint16_t bgColor = (sel && !enModoEdicionHorizontal) ? TFT_CYAN : TFT_BLACK;
    tft.fillRect(0, yp, tft.width(), 22, bgColor);

    uint16_t baseTextColor = (sel && !enModoEdicionHorizontal) ? TFT_BLACK : TFT_WHITE;
    tft.setTextColor(baseTextColor, bgColor);
    tft.setTextSize(2); 
    
    tft.setCursor(5, yp+3); 
    tft.print("E"); tft.print(idx+1);

    Etapa e = programas[programaEnEdicionIndex].etapas[idx];

    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 0) tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    else tft.setTextColor(baseTextColor, bgColor);
    tft.setCursor(50, yp+3); tft.print("R:"); tft.print(e.rampa);

    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 1) tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    else tft.setTextColor(baseTextColor, bgColor);
    tft.setCursor(130, yp+3); tft.print("T:"); tft.print(e.temperatura);

    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 2) tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    else tft.setTextColor(baseTextColor, bgColor);
