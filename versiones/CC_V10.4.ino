// =================================================================
// ==      CONTROLADOR DIGITAL PARA HORNO DE CERÁMICA V10.4       ==
// =================================================================
//      (Persistencia de Datos con LittleFS y Creación de Programas)
//
...
void setup() {
  ...
  if (!thermocouple.begin()) {
    Serial.println("MAX31855 no encontrado.");
  }

  // Inicializar LittleFS y cargar datos
...
void cargarConfiguracion() {
  if (!LittleFS.exists("/config.bin")) {
    Serial.println("Archivo de config no existe. Cargando default.");
...
void procesarEntrada(bool subiendo, bool bajando, bool ok, bool exit) {
    ...
    switch (estadoActual) {
      ...
      case MENU_AJUSTES:
        ...
        if (ok && menuAjustesCursor == 1) { valorCalibracionEditado = 0.0; estadoActual = MENU_CALIBRACION; }
        break;
      case MENU_CALIBRACION:
        ...
        if (ok) { 
          calibracionOffset = temperaturaCrudaSimulada - valorCalibracionEditado; 
          guardarConfiguracion();
      ...
      case MENU_CONFIG_PROG:
        if (enModoEdicionHorizontal) {
          ...
          if (subiendo || bajando) {
            int etIdx = configProgCursorVertical - 1, inc = (configProgCursorHorizontal == 1) ? 5 : ((configProgCursorHorizontal == 0) ? 10 : 1);
            ...
            else programas[programaEnEdicionIndex].etapas[etIdx].tiempo += (subiendo ? inc : -inc);
          }
        } else {
          ...
          if (exit) { 
              guardarConfiguracion(); 
              estadoActual = MENU_SELECCION_PROG; 
          }
          if (ok && configProgCursorVertical > 0) { enModoEdicionHorizontal = true; configProgCursorHorizontal = 0; }
        }
        break;
      case CALENTANDO: case MANTENIENDO: case ENFRIANDO: if (exit) estadoActual = STAND_BY; break;
    }
}
...
void dibujarLineaDeEtapa(int idx) {
  ...
  if (idx == -1) { tft.fillRect(0, yp, tft.width(), 30, (configProgCursorVertical == 0) ? TFT_CYAN : TFT_BLACK); tft.setTextColor((configProgCursorVertical == 0) ? TFT_BLACK : TFT_CYAN); tft.setCursor(10, yp + 5); tft.setTextSize(3); tft.print(programas[programaEnEdicionIndex].nombre); } 
  else {
    ...
    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 0) tft.setTextColor(TFT_YELLOW, TFT_BLACK); else tft.setTextColor(tx, bg);
    tft.setCursor(50, yp+3); tft.print("R:"); tft.print(e.rampa);
    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 1) tft.setTextColor(TFT_YELLOW, TFT_BLACK); else tft.setTextColor(tx, bg);
    tft.setCursor(130, yp+3); tft.print("T:"); tft.print(e.temperatura);
    if (sel && enModoEdicionHorizontal && configProgCursorHorizontal == 2) tft.setTextColor(TFT_YELLOW, TFT_BLACK); else tft.setTextColor(tx, bg);
