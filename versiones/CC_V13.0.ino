...
// VSPI: SCK 18, MISO 19, MOSI 23, SS 15 (TFT)
...
void actualizarLEDStatus() {
  ...
  switch(estadoActual) {
    ...
    case STAND_BY: 
      if (wifiConectado) setLEDColor(0, 0, 1); // AZUL (Online)
      else setLEDColor(0, 0, 0); // APAGADO (Offline)
      break;
...
void dibujarPantallaStandBy(bool r) {
  if (modoAP) {
    if (r) {
      ...
      tft.setTextColor(TFT_WHITE); tft.setCursor(20, 100); tft.print("Red WiFi: "); tft.print(apSSID);
      tft.setCursor(20, 130); tft.print("IP: 192.168.4.1");
    }
    return;
  }
...
void dibujarPantallaInfoWiFi() {
  ...
  tft.setTextColor(TFT_WHITE); tft.setCursor(20, 75); tft.print("IP: "); 
  if (WiFi.status() == WL_CONNECTED) tft.setTextColor(TFT_GREEN); else tft.setTextColor(TFT_RED);
  tft.print(WiFi.status() == WL_CONNECTED ? WiFi.localIP().toString() : (modoAP ? "CONFIG AP" : "DESC."));
  tft.setTextColor(TFT_WHITE); tft.setCursor(20, 110); tft.print("MAC: "); tft.print(WiFi.macAddress());
  tft.setCursor(20, 140); size_t total = LittleFS.totalBytes(); size_t used = LittleFS.usedBytes();
  tft.print("LittleFS: "); tft.print((float)used/total*100, 1); tft.print("%");
  tft.setCursor(20, 170); tft.print("V: 13.0 INDUSTRIAL");
  tft.setTextColor(TFT_YELLOW); tft.setCursor(20, 210); tft.print("OK para volver");
}
...
void iniciarWiFi() {
  ...
  if (wConfig.configurado) {
    WiFi.mode(WIFI_STA); WiFi.begin(wConfig.ssid, wConfig.pass);
    unsigned long st = millis();
    while (WiFi.status() != WL_CONNECTED && millis() - st < 10000) delay(500);
    if (WiFi.status() == WL_CONNECTED) { wifiConectado = true; return; }
  }
  iniciarModoAP();
}
